...
import { findFilePathByAttachmentId } from "Utils/helper";
...
class Dupload extends Component {
  ...
  async componentDidMount() {
    if (this.props.type === "attachmentId") {
      this.getImageByObject(this.props.data);
    } else {
      this.getFileList();
    }
  }
  ...
  UNSAFE_componentWillReceiveProps(nextProps) {
    if (this.props.listenerId !== nextProps.listenerId) {
      ...
      // 有的话就不需要重新显示图片
      !findObj && this.getImageByObject({ uid: nextProps.listenerId });
    }
  }
